...
if (isset($_GET['txtID'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM reingreso WHERE idreingreso = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sentencia_idusuario = $conexion->prepare("SELECT id_usuario FROM reingreso WHERE idreingreso = ?");
    ...
    $sentencia = $conexion->prepare("UPDATE reingreso SET 
        apellidopaterno = ?,
        apellidomaterno = ?,
        nombres = ?,
        alumnoextranjero = ?,
    ...
    if ($tmp_comprobantepago != '') {
        ...
        $sentencia = $conexion->prepare("SELECT comprobantepago FROM `nuevoingreso` WHERE idreingreso = ?");
        $sentencia->bind_param("i", $txtID);
        $sentencia->execute();
        $resultado = $sentencia->get_result();
